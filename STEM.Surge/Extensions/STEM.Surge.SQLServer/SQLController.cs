...
namespace STEM.Surge.SQLServer
{
    ...
    public class SQLController : STEM.Surge.DeploymentController
    {
        [Category("SQL Server")]
        ...
        public override List<string> ListPreprocess(IReadOnlyList<string> list)
        {
            ...
            try
            {
                ...
                {
                    SqlCommand command = new SqlCommand(String.Join("\r\n", Sql), connection);
            ...
            {
                PollError = "(" + STEM.Sys.IO.Net.MachineIP() + ") encountered an error querying " + Authentication.SqlDatabaseAddress + ": " + ex.Message;
                STEM.Sys.EventLog.WriteEntry("SQLController.ListPreprocess", ex.ToString(), STEM.Sys.EventLog.EventLogEntryType.Error);
            }
