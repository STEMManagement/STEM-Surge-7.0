...
namespace STEM.Surge.Compression
{
    ...
    public class Unzip : Instruction
    {
        [Category("Source")]
        [DisplayName("Source File"), DescriptionAttribute("The path and filename of the zip file to be unzipped.")]
        public string SourceFile { get; set; }

        [Category("Destination")]
        [DisplayName("Destination Path"), DescriptionAttribute("The location where the unzipped file(s) are to be placed.")]
                ...
                using (FileStream fs = File.Open(SourceFile, FileMode.Open, FileAccess.Read, FileShare.None))
                {
                    using (ZipInputStream zStream = new ZipInputStream(fs))
                    {
                        ...
                        while ((e = zStream.GetNextEntry()) != null)
                        {
                            ...
                            File.SetLastWriteTimeUtc(outputFile, e.DateTime);
                        }
                    }
                }
            }
