...
namespace STEM.Surge.GCS
{
    ...
    [Description("Write data to a container from a file in GCS.")]
    public class FileToContainer : STEM.Surge.Instruction
    {
        ...
        protected override bool _Run()
        {
            try
            {
                ...
                string container = Authentication.ContainerFromPath(SourceFile);
                ...
                {
                    Authentication.Client.DownloadObject(container, prefix, s);

                    bData = new byte[s.Position];
                    s.Position = 0;
